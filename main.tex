...
\begin{titlepage}
    \begin{center}
        ...
        % ---- Titre principal ----
        {\Large \textbf{Université de Toulouse}}\\[0.5em]
        ...
        \url{https://creativecommons.org/licenses/by-nc/4.0/}\\[1.5cm]
...
\begin{itemize}
    \item Spéciation allopatrique : elle repose sur un isolement géographique complet (séparation par une montagne, un rift, une mer, etc.), empêchant le flux de gènes entre les populations. (figure \ref{fig:speciation_models})
    \item Spéciation parapatrique : elle se produit entre des populations partiellement isolées, vivant dans des milieux aux conditions écologiques différentes. La sélection naturelle et la dérive génétique favorisent, la divergence malgré un flux génétique limité. (figure \ref{fig:speciation_models})
\end{itemize}
...
\begin{itemize}
    \item La spéciation sympatrique : sous certaines conditions, la spéciation sympatrique peut survenir grâce à la sélection disruptive et à l’apparition d’isolements reproductifs au sein d’une même population, dans un espace géographique partagé, par l’intermédiaire de polymorphismes génétiques et comportementaux (figure \ref{fig:speciation_models}) \parencite{maynardsmith1966sympatric}.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.5]{figures/speciation_allopatrie.png}
    \caption[Modèles de spéciation et flux de gènes]{
        \textbf{Continuum de la spéciation :} représentation schématique des principaux modèles de spéciation en fonction du degré d’isolement reproductif et du flux de gènes.
        (Gauche) \textit{Spéciation allopatrique} : isolement géographique complet (barrière physique) empêchant tout flux de gènes.
        (Centre) \textit{Spéciation parapatrique} : isolement partiel entre populations adjacentes, où le flux de gènes est limité mais non nul (barrière semi-perméable).
        (Droite) \textit{Spéciation sympatrique} : divergence au sein d’un même espace géographique, avec hybridation et flux de gènes résiduels (individus violets).
        La flèche verticale représente la progression temporelle de la divergence génétique et la transition d’un isolement géographique vers un isolement reproductif intrinsèque.
    }
    \label{fig:speciation_models}
\end{figure}
...
Les approches bayésiennes et de modélisation se sont imposées comme des outils clés pour tester différents scénarios de spéciation.
La méthode dite Approximate Bayesian Computation (ABC) vient simuler des jeux de données sous diverses hypothèses d’isolement ou de flux génétique, puis compare les résultats aux données observées pour identifier le scénario le plus plausible.
...
Les analyses confirment que les trois lignées principales (le loup gris (\textit{Canis lupus}), le loup de l’Est (\textit{Canis lycaon}) et le coyote (\textit{Canis latrans})) ont divergé il y a environ 60 à 70 000 ans.
Depuis cette séparation, de multiples événements d’admixture ont eu lieu. Le loup de l’Est apparaît comme une lignée distincte, ayant divergé du loup gris tout en présentant des traces d’échanges génétiques anciens avec le coyote, survenus il y a environ 37 000 ans (\ref{fig:canides_admixture}).
Malgré ces hybridations répétées, les analyses génomiques montrent une différenciation du loup de l’Est, confirmant son statut d’espèce.
Les coyotes de l’Est, résultent d’événements d’admixture plus récents entre coyotes de l’Ouest et loups de l’Est, survenus au cours de leur expansion vers le Canada et la côte atlantique.
Ces résultats révèlent un assemblage génomique complexe, où certaines régions du génome conservent la trace d’une divergence ancienne, tandis que d’autres témoignent d’introgressions récentes entre lignées apparentées.

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.5]{figures/Canis_latrans.png}
    \caption[Histoire évolutive des canidés nord-américains]{
        \textbf{Histoire évolutive et flux de gènes entre lignées de canidés nord-américains.}
        Schéma synthétique adapté de \textcite{vilaca-2023} illustrant la chronologie des événements d’admixture entre
        le loup gris (\textit{Canis lupus}), le loup de l’Est (\textit{Canis lycaon}) et le coyote (\textit{C. latrans}).
        Les flèches représentent les principaux épisodes d’échanges génétiques avec leurs dates.}
    \label{fig:canides_admixture}
\end{figure}
...
Ce cas emblématique de l'histoire humaine illustre que l’isolement reproductif entre espèces proches peut être perméable, même chez les hominidés.
...
Les plantes offrent un autre exemple intéressant de l’impact de l’admixture sur la spéciation.
C’est l’admixture qui a permis la diversification des plantes agricoles.
Par exemple, le maïs (\textit{Zea mays}) est issu d’un processus complexe d’hybridation entre différentes espèces sauvages du genre \textit{Zea}, notamment le téosinte (\textit{Zea mays ssp. parviglumis}) \parencite{mnhn2024mais}.
Cependant, ce processus peut également avoir un impact néfaste sur les écosystèmes : l’introgression adaptative peut se produire dans l’autre sens, comme lorsque des gènes du maïs ont permis à la téosinte de s’adapter au climat européen et de devenir envahissante \parencite{lecorre2020adaptive}.

Autre exemple : le blé (\textit{Triticum aestivum}) qui est un hybride allopolyploïde résultant de croisements entre plusieurs espèces de graminées, notamment \textit{Triticum urartu}, \textit{Aegilops speltoides} et \textit{Aegilops tauschii} \parencite{feldman2001origin, feldman2007domestication}.Ces hybridations ont permis l’introduction de gènes conférant des traits agronomiques avantageux, tels que la résistance aux maladies, l’adaptation aux conditions climatiques variées et l’amélioration des rendements.
...
\subsection{Les enjeux de ces études}
Ces trois travaux rappellent que l’admixture concerne l’ensemble du vivant, des animaux sauvages aux humains et aux plantes cultivées. Mais chacun répond à un enjeu spécifique.
L’article de \textcite{vilaca-2023} illustre le rôle de l’hybridation dans la gestion et la conservation de la biodiversité, en montrant comment les flux de gènes influencent la définition même des espèces et les politiques de protection associées.
Celui de \textcite{green2010neandertal} éclaire notre propre histoire évolutive : il révèle que \textit{Homo sapiens} est lui-même issu d’un métissage ancien, ce qui permet de mieux comprendre nos origines, nos traits adaptatifs et la diversité actuelle de l’espèce humaine.
Enfin, les travaux de \textcite{feldman2001origin,feldman2007domestication} démontrent que ces mêmes processus d’admixture et de sélection ont fondé les premières agricultures, à l’origine de la domestication des plantes et de l’émergence des sociétés sédentaires modernes.
Le cas du maïs (\textit{Zea mays}) illustre particulièrement bien cette dynamique : issu d’un processus complexe d’hybridation avec le téosinte (\textit{Zea mays ssp. parviglumis}) \parencite{mnhn2024mais}, il témoigne de l’importance évolutive et agronomique de l’admixture.
Mais ce mécanisme peut aussi avoir des effets inverses : des flux de gènes issus du maïs cultivé ont favorisé l’adaptation de la téosinte au climat européen, conduisant à l’émergence d’une population envahissante \parencite{lecorre2020adaptive}.
Ainsi, qu’il s’agisse de la conservation, de l’évolution humaine ou de la domestication végétale, l’admixture apparaît comme un moteur universel, à la fois créateur de diversité et source de défis écologiques.
...
Il est indéniable que les approches génomiques ont révolutionné l’étude de la spéciation et de l’admixture. En permettant d'accéder à la totalité des génomes, offrant une large vision de la diversité génétique, du flux de gènes et des mécanismes adaptatifs sous-jacents.

Contrairement aux marqueurs moléculaires traditionnels, qui ne capturent qu'une fraction limitée de la variation génétique, les génomes entiers permettent d'identifier des signatures fines d'introgression et de divergence. la bioinformatique a permis de cartographier ces génomes, donc de discriminer à cette échelle, révélant des zones de forte différenciation (barrières reproductives) et des régions perméables au flux de gènes, donc des régions associées à des gènes sous sélection. \parencite{seehausen2014genomics,roux2016continuum}

les méthodes actuelles, telles que les D-statistics(Test ABBA - BABA), MSMC2, Admixtur, les GEA et ABC-RF, offrent une capacité à dater des événements d’admixture et à reconstruire l’histoire démographique des populations. Elles permettent de quantifier précisément les proportions d’introgression, de modéliser des scénarios complexes de spéciation avec flux de gènes, et d’estimer les temps de divergence avec une résolution temporelle fine \parencite{schiffels2014msmc,pudlo2016abc}.
Les méthodes bayésiennes et de simulation permettent également de tester rigoureusement des scénarios évolutifs, en intégrant l’incertitude et la variabilité des données génomiques, permettant d'estimer la vraisemblance des modèles proposés. \parencite{pudlo2016abc,raynal2019abc}

La figure~\ref{fig:abc_rf_model} illustre un exemple de modèle coalescent utilisé dans ce cadre, permettant d’estimer simultanément les temps de divergence, les flux de gènes et les tailles efficaces de population.
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.5]{figures/ABC_RF_model.png}
    \caption[Modèle coalescent et inférence bayésienne ABC-RF]{
        \textbf{Modèle évolutif de quatre populations humaines utilisé pour l’inférence bayésienne des paramètres démographiques par ABC-RF.}
        Les couleurs représentent les tailles efficaces de population et les épisodes de goulots d’étranglement.
        Ce modèle illustre la capacité des approches génomiques couplées à des méthodes d’apprentissage automatique à reconstruire les scénarios d’admixture et de divergence.
        D’après \textcite{raynal2019abc}.
    }
    \label{fig:abc_rf_model}
\end{figure}
...
Malgré leurs avancées, les approches génomiques présentent plusieurs limites méthodologiques importantes. Tout d’abord, la qualité et la couverture des données génomiques influencent fortement la fiabilité des inférences. Des séquences incomplètes ou de faible profondeur peuvent introduire des biais dans l’estimation des fréquences alléliques, affectant ainsi les tests d’introgression et les modèles démographiques \parencite{han2014characterizing}. De plus l'échantillonnage des populations est limité, pouvant conduire une sous-estimations de la diversité de nos résultats, en particulier des especes éloignées des especes modeles.
...
L’étude de l’ADN ancien permet d’observer directement les flux de gènes au cours du temps, en retraçant les événements d’admixture passés plutôt que de les inférer uniquement à partir des génomes contemporains \parencite{green2010neandertal, prufer2014complete}.
...
L’intégration de la dimension écologique et des modèles éco-évolutifs (qui relient environnement, sélection et démographie) constitue une voie prometteuse.
Ces approches visent à comprendre comment les gradients écologiques, les pressions de sélection locales et la connectivité des habitats façonnent la combinaison génomique.
Enfin, les développements récents en modélisation bayésienne hiérarchique et en apprentissage profond (Deep learning) laissent entrevoir une automatisation partielle de l’inférence démographique, tout en intégrant la complexité des paysages génétiques et environnementaux.

En résumé, les approches génomiques ont profondément transformé notre compréhension de la spéciation et de l’admixture.
Elles offrent une approche quantitative et dynamique des échanges génétiques, mais exigent une interprétation prudente et contextualisée.
Le futur de la discipline repose sur la combinaison de la génomique, de l’écologie et de la modélisation, pour appréhender la diversification du vivant comme un processus intégratif et multidimensionnel.
...
Les exemples issus des canidés nord-américains, de l’espèce humaine et des plantes cultivées (comme le maïs et la téosinte) illustrent la généralité de ces processus à travers les règnes, montrant que l’admixture agit aussi bien comme moteur d’adaptation que comme facteur de déséquilibre écologique.
Ces avancées ont été rendues possibles par le développement de la bioinformatique et de la génomique, qui ont fourni des tests robustes pour détecter et quantifier l’admixture, tels que les D-statistiques (test ABBA–BABA).
...
En somme, l’étude de l’admixture renouvelle notre compréhension de la spéciation, en montrant qu’elle est moins un événement discret qu’un continuum créé par les flux génétiques à différentes échelles temporelles et taxonomiques.
